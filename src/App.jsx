...
const GeoButton = ({ children, onClick, type = "button", disabled = false, className = "", wFull = false, icon: Icon, variant = "dark" }) => {
  ...
  return (
    <button
      ...
      disabled={disabled}
      onClick={onClick}
      className={`group relative ${baseClasses} px-8 py-4 rounded-xl font-medium text-sm sm:text-base overflow-hidden transition-shadow duration-300 flex items-center justify-center gap-2 disabled:opacity-70 ${wFull ? 'w-full' : 'w-full sm:w-auto'} ${className}`}
    >
      <span className={`relative z-10 flex items-center justify-center gap-2 w-full transition-colors duration-300 group-hover:text-white`}>
        {children}
...
const HomePage = ({ setCurrentPage, openContactModal }) => {
  ...
  const testimonials = [
    ...
    {
      quote: "Their radical transparency is a breath of fresh air. No hidden commissions, no pushed products. The step-up SIP strategy has completely transformed our retirement timeline.",
      name: "Ananya R.",
      ...
      <section className="py-24 sm:py-32 lg:py-40 bg-white px-6 sm:px-10 lg:px-16 xl:px-24 text-left border-b border-zinc-200/50">
        <div className="w-full max-w-[1800px] mx-auto">
          <FadeIn direction="up" className="mb-16 max-w-3xl">
              ...
              <p className="text-base sm:text-lg lg:text-xl text-zinc-600 font-light leading-relaxed">
                Hear from the professionals, business owners, and families who have entrusted us with their financial architecture.
              </p>
          </FadeIn>
...
const AskGeoApp = () => {
  ...
  return (
    <div className="min-h-screen bg-zinc-50 text-zinc-900 font-sans selection:bg-emerald-200 selection:text-zinc-900 overflow-x-hidden text-left">
      ...
      {/* --- PAGE ROUTER MAIN --- */}
      <main>
        {currentPage === 'home' && <HomePage setCurrentPage={setCurrentPage} openContactModal={openContactModal} />}
